...
def calcMove(circles, startPosition, selfAngle, endPoint, needToDraw, targetDeep) :
    ...
    if isinstance(path[0], AStar.Arcc):
        ...
        return STATE_OK, moveAlongArcc(startPosition, dr, path[0].p1, path[0].r,
                                       angle, turn, polarLenght, tangent, selfAngle, needToStop, targetDeep), image

    #print("Error: unknow error")
    return ERROR_STATE, [], image

def moveAlongArcc(startPosition, dr, polarAngle, r, angle, turn, polarLenght, tanget, selfAngle, needToStop, targetDeep) :
    ...
    if turn == RIGHT: return normVectorFromArcc([[vx, vy, vz], [0, 0, angle]], needToStop)
    return normVectorFromArcc([[vx, vy, vz], [0, 0, -angle]], needToStop)

def moveToLine(startPosition, length, angle, turn, angleToPoint, needToStop, targetDeep) :
    # движемся по линии
    #print('LINE')
    ...
    if turn == RIGHT: return normVectorFromLine([[vx, vy, vz], [0, 0, angle]], needToStop)
    return normVectorFromLine([[vx, vy, vz], [0, 0, -angle]], needToStop)
